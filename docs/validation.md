...
Spice Route 1.5.0 has native Windows x64 and ARM64 packages plus macOS Apple Silicon and Intel packaging. Current checks pass 107 Rust tests, 13 headless native engine-client contract checks, and 24 frontend tests. The shared engine covers compatibility, selective history transfer, snapshot capture, conflicts, project reconstruction, diagnostics, and recovery. See the [1.5.0 desktop verification record](testing-1.5.0.md) for package evidence and remaining physical-device checks.
