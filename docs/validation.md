...
Spice Route 1.4.1 for Windows builds on the 0.3.2 Rust engine. Current checks pass 92 Rust tests, 11 headless native engine-client contract checks, and 24 Tauri frontend tests. The engine adds buffered manifest reads, lazy page inventories, deterministic settings validation, reusable content capture, bounded progress reporting, hidden subprocesses, cheaper recovery checks, and reviewed cloud replacement when a device has no saved baseline. See the [performance audit](performance-audit.md), [maintenance verification record](testing-0.3.2.md), and [Windows verification record](testing-1.4.1.md) for measured results and remaining interactive and live-device acceptance.
