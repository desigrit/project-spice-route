...
The Mac reported state/history migrations 57/6. Codex can migrate the two databases independently. Spice Route 1.6.3 recognizes this pair only when the state database exactly matches the tested 57 layout and the history database exactly matches the tested 6 layout, including indexes, triggers, and completed migrations. Its combined fingerprint is `dac383732eddc28f1d2faca9a5b107ac23877db8dff8a6903bfd79b918a386c4`. The reported migration numbers do not prove that the Mac has this exact layout; a different fingerprint remains blocked and requires a compatibility report. Disposable tests cover 57/6 export, exclusion, repeated restore, and transfers to and from 57/7. A 57/6 destination refuses non-null history lifecycle fields introduced in history migration 7 before changing either database.
