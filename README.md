...
[Windows x64](artifacts/Spice-Route-1.5.1-windows-x64-setup.exe) · [Windows ARM64](artifacts/Spice-Route-1.5.1-windows-arm64-setup.exe) · [macOS Apple Silicon](artifacts/macos-apple-silicon/Spice-Route-1.5.1-macos-apple-silicon.dmg) · [macOS Intel](artifacts/macos-intel/Spice-Route-1.5.1-macos-intel.dmg) · [Getting started](#getting-started) · [Report an issue](https://github.com/desigrit/project-spice-route/issues)
...
**The current desktop version is 1.5.1.** Windows now has separate native x64 and ARM64 packages. The macOS app uses the same Rust engine and snapshot format, with packages for Apple Silicon and Intel generated by the desktop build workflow. This release supports Codex database schema 55, translates attachments between tested formats, and recognizes equivalent Codex schemas created with Windows or macOS line endings. Install the same Spice Route version on every computer before exchanging snapshots. Interactive testing and a live transfer between devices remain validation steps, so keep an independent backup of work you cannot replace.
...
- [Windows x64](artifacts/Spice-Route-1.5.1-windows-x64-setup.exe) for Intel and AMD computers
- [Windows ARM64](artifacts/Spice-Route-1.5.1-windows-arm64-setup.exe) for Windows on Arm computers
...
See the [compatibility design](docs/compatibility-plan.md) and [1.5.1 testing notes](docs/testing-1.5.1.md) for the exact boundaries. The verification guide records the current Rust, frontend, native engine-client, and package checks. Real-device history display, continuation, and cloud-client behavior remain part of manual acceptance.
...
Both interfaces use the 1.5.1 engine for compatibility checks, snapshot capture, transfer performance, settings validation, progress reporting, diagnostics, and recovery. See the [performance audit](docs/performance-audit.md) for measured background.
...
The script produces `artifacts/Spice-Route-1.5.1-windows-x64-setup.exe` and its checksum. Pass `-Architecture arm64` for the native Windows on Arm package. It builds and packages the application without opening it when `-SkipStartupProbe` is supplied. See the [1.5.1 verification guide](docs/testing-1.5.1.md) for engine-client tests and interactive acceptance checks.
...
```bash
bash ./scripts/build-macos.sh aarch64-apple-darwin 1.5.1 apple-silicon
```
...
The [desktop build workflow](.github/workflows/desktop-builds.yml) compiles Windows x64, Windows ARM64, macOS Apple Silicon, and macOS Intel packages from version tags. The [1.5.1 release run](https://github.com/desigrit/project-spice-route/actions/runs/35435536698) completed successfully for all four targets.
