...
**The current desktop version is 1.5.0.** Windows now has separate native x64 and ARM64 packages. The macOS app uses the same Rust engine and snapshot format, with packages for Apple Silicon and Intel generated by the desktop build workflow. This release supports Codex database schema 55 and translates attachments when transferring between tested older and newer formats. Install the same Spice Route version on every computer before exchanging snapshots. Interactive testing and a live transfer between devices remain validation steps, so keep an independent backup of work you cannot replace.
...
- [Windows x64](artifacts/Spice-Route-1.5.0-windows-x64-setup.exe) for Intel and AMD computers
- [Windows ARM64](artifacts/Spice-Route-1.5.0-windows-arm64-setup.exe) for Windows on Arm computers
...
See the [compatibility design](docs/compatibility-plan.md) and [1.5.0 testing notes](docs/testing-1.5.0.md) for the exact boundaries. The verification guide records the current Rust, frontend, native engine-client, and package checks. Real-device history display, continuation, and cloud-client behavior remain part of manual acceptance.
...
Both interfaces use the 1.5.0 engine for compatibility checks, snapshot capture, transfer performance, settings validation, progress reporting, diagnostics, and recovery. See the [performance audit](docs/performance-audit.md) for measured background.
...
The script produces `artifacts/Spice-Route-1.5.0-windows-x64-setup.exe` and its checksum. Pass `-Architecture arm64` for the native Windows on Arm package. It builds and packages the application without opening it when `-SkipStartupProbe` is supplied. See the [1.5.0 verification guide](docs/testing-1.5.0.md) for engine-client tests and interactive acceptance checks.
...
```bash
bash ./scripts/build-macos.sh aarch64-apple-darwin 1.5.0 apple-silicon
```
